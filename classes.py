...
class ThreadsSaveData(GetData):
    ...
    def __read_file(self) -> None:
        ...
        with self.__read_thread_cond:
            ...
            if self.file_name in DICT_FILES.items():
                with open(self.file_name, 'r', encoding='utf-8') as outfile:
                    print(f'Файл прочитан: {self.file_name}, всего записей: {outfile.__sizeof__()}')

    def __init__(self, news_data: list) -> None:
        super().__init__(news_data)
        self.__urls_thread: Thread = Thread(target=self.__urls_save_to_file, daemon=True)
        self.__photos_thead: Thread = Thread(target=self.__photos_save_to_file, daemon=True)
        self.__text_thread: Thread = Thread(target=self.__text_save_to_file, daemon=True)
        self.__read_thread: Thread = Thread(target=self.__read_file, daemon=True)
